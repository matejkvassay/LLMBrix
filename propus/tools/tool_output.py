...
from propus.msg.tool_result_msg import ToolResultMsg
...
class ToolOutput(BaseModel):
    ...
    def to_tool_msg(self, tool_call: types.FunctionCall) -> ToolResultMsg:
        """
        Converts this tool execution output into a ToolResultMsg.
        Args:
            tool_call: FunctionCall tool call request related to this tool execution output.
        Returns: ToolResultMsg
        """
        llm_payload = self.result.copy()
        if self.error:
            llm_payload["error"] = self.error
        return ToolResultMsg(tool_call=tool_call, result=llm_payload)
